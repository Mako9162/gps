...
function regidController(req, res) {
  ...
  jwt.verify(req.token, "smvssmvs", (err, authData) => {
    if (err) {
      ...
      sqls = `SELECT * FROM usuapi WHERE usuario= '${usuario}' and est=1`;
        ...
        if (err) {
          ...
          db.query(sqls, (err, result) => {
            if (!result.length) {
              ...
              const sql2 =
                "SELECT * FROM equipo WHERE equi_vehi_id= ? AND equi_imei= ?";
              db.query(sql2, [r_patente, imei], (error, result) => {
                if (result.length > 0) {
                  ...
                  const fechaPosi = moment(
                    fechap,
                    "YYYY-MM-DD HH:mm:ss",
                    true
                  ).isValid();
                  const fechaEnvi = moment(
                    fechar,
                    "YYYY-MM-DD HH:mm:ss",
                    true
                  ).isValid();
                  ...
                  const fechaPosi1 = moment(
                    fechap1,
                    "YYYY-MM-DD HH:mm:ss",
                    true
                  ).isValid();
                  const fechaEnvi1 = moment(
                    fechar1,
                    "YYYY-MM-DD HH:mm:ss",
                    true
                  ).isValid();
